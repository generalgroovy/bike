...
test('sustained overload causes an explicit service breach instead of an invisible loss',()=>{
  ...
  assert.ok(g.runStats.districtBreaches>=1);assert.equal(g.reputation,before-5);const state=g.districtPressure('mitte');assert.ok(state.breaches>=1);assert.equal(g.dispatchLog.at(-1).action,'service-breach');
});
