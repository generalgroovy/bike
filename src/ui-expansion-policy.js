...
const stageEl=dialog.querySelector('.policy-stage'),grid=dialog.querySelector('.policy-grid');let shownKey='';
function open(){dialog.hidden=false;if(!dialog.open){if(typeof dialog.showModal==='function')dialog.showModal();else dialog.setAttribute('open','');}}
function close(){if(dialog.open&&typeof dialog.close==='function')dialog.close();else dialog.removeAttribute('open');dialog.hidden=true;}
function render(){const game=Game.lastInstance,pending=game?.expansionPolicyPending;if(!game||!pending){if(!dialog.hidden)close();shownKey='';return;}const key=`${pending.level}:${pending.stage}`;if(key===shownKey&&dialog.open)return;shownKey=key;stageEl.textContent=`${pending.stage} is live. Choose one operating doctrine for the larger territory.`;grid.replaceChildren();for(const policy of game.expansionPolicyChoices()){const button=document.createElement('button');button.dataset.policy=policy.id;const title=document.createElement('strong'),desc=document.createElement('span');title.textContent=policy.title;desc.textContent=policy.desc;button.append(title,desc);grid.append(button);}open();}
grid.addEventListener('click',event=>{const button=event.target.closest('[data-policy]'),game=Game.lastInstance;if(!button||!game)return;if(game.applyExpansionPolicy(button.dataset.policy)){close();shownKey='';}});
