...
constructor({seed=createSeed(),width=BERLIN.width,height=BERLIN.height}={}){
  ...
  this.runStats={peakActive:0,peakRadio:0,distance:0,riderChoices:0,radioDenied:0,eventExposure:0,breaks:0,toolsUsed:0,expansions:0};this.dispatchLog=[];
...
generateBerlin(){
  ...
  this.nodeMap=new Map(this.nodes.map(n=>[n.id,n]));this.edgeMap=new Map();for(const e of this.edges)this.edgeMap.set(this.edgeKey(e.a,e.b),e);this.graph=createGraphIndex(this.nodes,this.edges);this.depotNodeId=this.landmarks.find(l=>l.id==='checkpoint')?.addressNodeId??this.addressNodes.find(n=>n.unlockLevel===1)?.id??this.addressNodes[0]?.id;
}
...
deliveryById(id){return this.deliveries.find(d=>d.id===id);}
courierById(id){return this.couriers.find(c=>c.id===id);}
...
playableAddressNodes(){return this.addressNodes.filter(n=>(n.unlockLevel??1)<=this.cityLevel);}
...
routeBetween(startId,goalId){return shortestPathIndexed(this.graph,startId,goalId,this.edgeCost.bind(this));}
...
maybeAdvanceCity(){let next=this.cityLevel;for(const stage of this.unlockStages)if(this.completed>=stage.threshold)next=Math.max(next,stage.level);if(next<=this.cityLevel)return false;this.cityLevel=next;this.cityChangedAt=this.elapsed;this.radioSlots+=1;this.dispatchFocusMax+=1;this.dispatchFocus=Math.min(this.dispatchFocusMax,this.dispatchFocus+1);this.runStats.expansions+=1;const stage=this.currentStage();this.logDispatch('city-expand',null,{stage:stage.name,level:stage.level});this.flash(`CITY EXPANDED · ${stage.name} · ${stage.desc}`,7);return true;}
