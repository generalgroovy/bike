...
const clientPitch={kitchen:65,clinic:74,office:67,market:57,studio:72,workshop:62,client:60};
...
function clientCue(kind='client'){if(!audio.enabled||!audio.allow(`client-${kind}`,.12))return;audio.tone({midi:clientPitch[kind]??60,volume:.022,duration:.07,type:kind==='workshop'?'triangle':'sine',when:audio.beat(4),filter:kind==='market'?2200:4300});}
...
function tick(){const next=Game.lastInstance;if(!next)return;if(next!==game)reset(next);processLogs();selectionCues();demandCue();metricCues();const flow=exposeFlow();if(document.hidden||game.paused||game.gameOver||!audio.enabled)return;const t=now();pingDeliveries(t);riderNotes(t);pressureTone(t);audio.ambience({phase:lastDemandPhase??'quiet',pressure:typeof game.cityPressure==='function'?game.cityPressure():0,flow});}
registerUiTask('sensory-observer',tick,{interval:90,hiddenInterval:1800});

function hoverTarget(event){return event.target.closest?.('.task-card,.rider-card,.goal-card,#event-chip,#inspect-client,[data-channel],[data-tool],[data-speed],.client-hub-chip,.demand-rhythm,.service-load,.district-brief,.map-tools button,.time-tools button,.top-actions button,.brand,.shift-chip,.city-chip,.metrics>div');}
document.addEventListener('pointerover',event=>{const target=hoverTarget(event);if(!target)return;const key=target.dataset.delivery?`job:${target.dataset.delivery}`:target.dataset.courier?`rider:${target.dataset.courier}`:target.dataset.channel?`channel:${target.dataset.channel}`:target.dataset.tool?`tool:${target.dataset.tool}`:target.id||String(target.className);if(key===lastHovered)return;lastHovered=key;if(target.id==='inspect-client')clientCue(target.dataset.kind);else if(target.dataset.delivery&&target.dataset.clientKind)clientCue(target.dataset.clientKind);else if(target.dataset.delivery)audio.cue('hover-job');else if(target.dataset.courier){const i=game?.couriers.findIndex(c=>c.id===target.dataset.courier)??0;audio.cue('hover-rider',{riderIndex:Math.max(0,i)});}else if(target.dataset.channel)audio.cue(`hover-radio-${target.dataset.channel}`);else if(target.dataset.tool)audio.cue(`hover-tool-${target.dataset.tool}`);else if(target.classList.contains('shift-chip'))audio.cue('hover-shift');else if(target.classList.contains('city-chip'))audio.cue('hover-city');else if(target.classList.contains('service-load'))audio.cue('hover-load');else if(target.classList.contains('goal-card'))audio.cue('hover-goal');else audio.cue('ui-hover');});
