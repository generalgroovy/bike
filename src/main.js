...
deliveriesEl.addEventListener('click',(event)=>{const article=event.target.closest('[data-delivery]');if(!article)return;const delivery=game.deliveryById(article.dataset.delivery);if(!delivery)return;const channelButtonEl=event.target.closest('[data-channel]');if(channelButtonEl){const previousChannel=delivery.channel;const requested=channelButtonEl.dataset.channel;const changed=game.setChannel(delivery.id,requested);if(changed&&requested!=='off'&&previousChannel&&previousChannel!==delivery.channel)game.invalidateDeliberations(delivery.id);renderUI();return;}if(event.target.closest('[data-inspect]')){game.selectDelivery(delivery.id);renderUI();}});
