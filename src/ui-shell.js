...
const root=document.documentElement,actions=document.querySelector('.top-actions'),summary=document.querySelector('.task-summary'),dockHead=document.querySelector('.dock-head');
...
function makeButton(id,label,tip){const button=document.createElement('button');button.id=id;button.type='button';button.className='ghost ui-icon-button';button.textContent=label;button.dataset.tip=tip;button.setAttribute('aria-label',tip);return button;}
...
function applyDensity(){root.dataset.density=density;densityButton.textContent=density==='compact'?'▦':'▤';densityButton.classList.toggle('active',density==='compact');densityButton.dataset.tip=`Information density: ${density}. Press D to toggle.`;}
function applyFocus(){root.dataset.mapFocus=String(mapFocus);focusButton.classList.toggle('active',mapFocus);focusButton.textContent=mapFocus?'⌗':'⌗';focusButton.dataset.tip=mapFocus?'Map focus on · show rider dock (M)':'Map focus off · hide rider dock (M)';}
