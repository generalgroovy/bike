...
function reset(next){game=next;logIndex=next.dispatchLog.length;deliveryPings.clear();riderMotion.clear();lastSelectedDelivery=next.selectedDeliveryId;lastSelectedCourier=next.selectedCourierId;lastPressureCue=0;}
function tick(){const next=Game.lastInstance;if(!next)return;if(next!==game)reset(next);const t=now();processLogs();selectionCues();if(!game.paused&&!game.gameOver){pingDeliveries(t);riderNotes(t);pressureTone(t);}}
setInterval(tick,90);

document.addEventListener('pointerover',event=>{const target=event.target.closest?.('.task-card,.rider-card,.goal-card,#event-chip,[data-channel],[data-tool],[data-speed],.client-hub-chip,.demand-chip');if(!target)return;const key=target.dataset.delivery?`job:${target.dataset.delivery}`:target.dataset.courier?`rider:${target.dataset.courier}`:target.dataset.channel?`channel:${target.dataset.channel}`:target.dataset.tool?`tool:${target.dataset.tool}`:target.id||target.className;if(key===lastHovered)return;lastHovered=key;if(target.dataset.delivery)audio.cue('hover-job');else if(target.dataset.courier){const i=game?.couriers.findIndex(c=>c.id===target.dataset.courier)??0;audio.cue('hover-rider',{riderIndex:Math.max(0,i)});}else audio.cue('ui-hover');});
document.addEventListener('pointerout',event=>{if(!event.relatedTarget?.closest?.('.task-card,.rider-card,.goal-card,#event-chip,[data-channel],[data-tool],[data-speed],.client-hub-chip,.demand-chip'))lastHovered='';});
document.addEventListener('click',event=>{const target=event.target.closest?.('[data-tool],[data-speed],#pause,#help,#zoom-in,#zoom-out,#zoom-reset');if(!target)return;audio.cue(target.dataset.tool?`tool-${target.dataset.tool}`:'ui-click');});
