...
Game.prototype.deliveryFeasibility=function(delivery,{horizon=180}={}){
  ...
  const remaining=Math.max(0,delivery.deadlineAt-this.elapsed),candidates=this.couriers.map(rider=>{const availability=this.courierAvailability(rider,delivery);if(!availability||!Number.isFinite(availability.arrivalIn)||availability.arrivalIn>horizon)return null;const loaded=loadedTravelTime(this,rider,delivery),pickupWait=delivery.pickupReadyAt==null?0:Math.max(0,delivery.pickupReadyAt-(this.elapsed+availability.arrivalIn)),finishIn=availability.arrivalIn+pickupWait+loaded,margin=remaining-finishIn;return{rider,state:availability.state,availableNow:availability.availableNow,arrivalIn:availability.arrivalIn,pickupWait,loadedIn:loaded,finishIn,margin};}).filter(Boolean).sort((a,b)=>b.margin-a.margin||a.finishIn-b.finishIn);
