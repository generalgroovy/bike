...
function addressLabel(delivery,key){return delivery?.[`${key}Address`]?.label??game.nodeById(delivery?.[`${key}Id`])?.name??'Berlin';}
...
function updateJobElement(el,d){
  const type=DELIVERY_TYPES[d.type],remaining=d.deadlineAt-game.elapsed,claimed=d.status==='claimed',nearest=game.nearestIdleDistance(d),channel=d.called?d.channel:'off';
  el.style.setProperty('--type',type.color);el.dataset.channel=channel;el.dataset.claimed=String(claimed);setText(el.querySelector('.job-glyph'),d.pickedUp?'◎':type.glyph);setText(el.querySelector('.job-id'),d.id.toUpperCase());setText(el.querySelector('.job-type'),type.label);setText(el.querySelector('.job-route'),`${addressLabel(d,'pickup')} → ${addressLabel(d,'dropoff')}`);
  ...
  const routeKm=kmFromWorld(d.routeDistance??0);setText(el.querySelector('.job-meta'),`${d.distanceTier??'CITY'} · ${routeKm} · ${state} · €${d.reward}`);const time=el.querySelector('.job-time');setText(time,formatTime(remaining));setClass(time,`job-time${remaining<18?' urgent':''}`);
