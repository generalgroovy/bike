...
function render(){
  ...
  for(const item of outlook){const chip=document.createElement('span');chip.className='outlook-chip';chip.dataset.state=item.state;const name=document.createElement('b');name.textContent=item.rider.name;const state=document.createElement('em');state.textContent=item.availableNow?'NOW':`${item.state.toUpperCase()} · ${formatTime(item.arrivalIn)}`;chip.append(name,state);root.append(chip);}
}
