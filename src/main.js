...
const inspector=$('#job-inspector'),inspectGlyph=$('#inspect-glyph'),inspectId=$('#inspect-id'),inspectSpecial=$('#inspect-special'),inspectPickup=$('#inspect-pickup'),inspectDropoff=$('#inspect-dropoff'),inspectTime=$('#inspect-time'),inspectDistance=$('#inspect-distance'),inspectReward=$('#inspect-reward'),inspectLikely=$('#inspect-likely'),inspectStreets=$('#inspect-streets');
const helpPanel=$('#help-panel'),hoverTip=$('#hover-tip'),upgradeModal=$('#upgrade-modal'),upgradeChoices=$('#upgrade-choices'),gameoverModal=$('#gameover-modal'),summaryEl=$('#run-summary'),bestEl=$('#best-score'),reviewMetrics=$('#review-metrics'),reviewAdvice=$('#review-advice');
...
function updateTaskElement(el,d){
  const type=DELIVERY_TYPES[d.type],remaining=d.deadlineAt-game.elapsed,claimed=d.status==='claimed',channel=d.called?d.channel:'off',likely=likelyRiders(d,1)[0]?.c;
  el.style.setProperty('--type',type.color);el.dataset.channel=channel;el.dataset.claimed=String(claimed);el.dataset.urgent=String(remaining<18);el.classList.toggle('selected',game.selectedDeliveryId===d.id);
...
function renderEvent(){const ev=game.currentEvent;if(!ev){eventChip.hidden=true;return;}eventChip.hidden=false;eventChip.dataset.state=ev.state;setText(eventState,ev.state==='active'?'ACTIVE':'FORECAST');setText(eventTitle,`${ev.title} · ${ev.place}`);setText(eventDetail,ev.state==='active'?(ev.advisory?'Advisory live · riders avoid the affected street.':'Street is slow · riders reroute at intersections.'):ev.detail);setText(eventTime,ev.state==='active'?`${formatTime(ev.endsAt-game.elapsed)} left`:`in ${formatTime(ev.startsAt-game.elapsed)}`);eventAdvisory.disabled=ev.advisory||game.dispatchFocus<1;eventAdvisory.dataset.used=String(Boolean(ev.advisory));setText(eventAdvisory,ev.advisory?'DETOUR ADVISORY LIVE':'DETOUR · 1 FOCUS');}

function renderInspector(){
  const d=game.deliveryById(game.selectedDeliveryId);if(!d||!game.activeDeliveries().includes(d)){inspector.hidden=true;return;}inspector.hidden=false;const type=DELIVERY_TYPES[d.type],remaining=d.deadlineAt-game.elapsed,likely=likelyRiders(d,2).map(x=>x.c.name).join(' · ')||'no free rider';setText(inspectGlyph,type.glyph);inspectGlyph.style.color=type.color;setText(inspectId,d.id.toUpperCase());inspectSpecial.hidden=!d.specialLabel;setText(inspectSpecial,d.specialLabel??'');setText(inspectPickup,d.pickupAddress);setText(inspectDropoff,d.dropoffAddress);setText(inspectTime,`${formatTime(remaining)} left`);setText(inspectDistance,formatKm(d.plannedDistance));setText(inspectReward,`€${d.reward}`);setText(inspectLikely,`likely: ${likely}`);setText(inspectStreets,d.plannedStreets?.length?d.plannedStreets.slice(0,8).join(' → '):'route pending');const state=game.deliveryToolState(d.id);for(const button of inspector.querySelectorAll('[data-tool]'))button.disabled=!state?.[button.dataset.tool];}
...
function showGameOver(){const s=game.summary(),best=Math.max(Number(readLocal('sendit.bestScore','0'))||0,s.score);writeLocal('sendit.bestScore',best);setText(bestEl,best.toLocaleString());summaryEl.replaceChildren();for(const[value,label]of[[s.score.toLocaleString(),'Score'],[s.completed,'Delivered'],[s.failed,'Missed'],[s.review.stage,'Area'],[s.review.toolsUsed,'Tools used'],[s.seed,'Seed']]){const box=document.createElement('div'),strong=document.createElement('strong'),span=document.createElement('span');strong.textContent=String(value);span.textContent=label;box.append(strong,span);summaryEl.append(box);}reviewMetrics.replaceChildren();for(const[value,label]of[[s.review.neverCalled,'Never called'],[s.review.calledUnclaimed,'No taker'],[s.review.claimedLate,'Accepted late'],[s.review.radioDenied,'Radio blocked'],[s.review.breaks,'Breaks'],[s.goals,'Goals']]){const box=document.createElement('div'),strong=document.createElement('strong'),span=document.createElement('span');strong.textContent=value;span.textContent=label;box.append(strong,span);reviewMetrics.append(box);}reviewAdvice.replaceChildren();const top=document.createElement('p');top.textContent=`Top rider: ${s.review.topRider}`;reviewAdvice.append(top);for(const advice of s.review.advice){const p=document.createElement('p');p.textContent=advice;reviewAdvice.append(p);}openDialog(gameoverModal);}
...
function taskTip(d,x,y){if(!d)return;const type=DELIVERY_TYPES[d.type],nearest=likelyRiders(d,2).map(v=>v.c.name).join(', ')||'none free';showTip(`${d.id.toUpperCase()} · ${type.label}${d.specialLabel?` · ${d.specialLabel}`:''}`,[`${d.pickupAddress} → ${d.dropoffAddress}`,`${formatKm(d.plannedDistance)} · €${d.reward} · ${formatTime(d.deadlineAt-game.elapsed)} left`,`Likely riders: ${nearest}`,d.specialDesc,d.plannedStreets?.slice(0,6).join(' → ')],x,y,`task:${d.id}`);}
function riderTip(c,x,y){if(!c)return;const d=c.deliveryId?game.deliveryById(c.deliveryId):null,predicted=game.predictCall(c),progress=Math.round(game.courierTaskProgress(c)*100),energy=Math.round((1-c.fatigue)*100);showTip(`${c.name} · ${c.experience.title}`,[`${c.personality.icon} ${c.personality.title}: ${c.personality.desc}`,`Location: ${riderLocation(c)}`,d?`${d.id.toUpperCase()} · ${progress}% · ETA ${formatTime(game.courierETA(c)??0)}`:predicted?`Likely to consider ${predicted.delivery.id.toUpperCase()}`:'Listening',`Energy ${energy}%${c.phase==='break'?` · back ${formatTime(game.breakRemaining(c))}`:''}`],x,y,`rider:${c.id}`);}
...
eventAdvisory.addEventListener('click',()=>{if(game.issueDetourAdvisory())renderUI();});
