...
const options={snapTolerance:Number(args.get('snap-tolerance')??1.5),maxDistance:Number(args.get('address-snap')??28),maxComponents:Number(args.get('max-components')??12),minLargestShare:Number(args.get('min-largest-share')??.9),minAddressMatch:Number(args.get('min-address-match')??.8)};
...
console.log(`Largest component ${(quality.largestShare*100).toFixed(1)}% · address match ${(quality.addressMatch*100).toFixed(1)}%`);
