...
test('Berlin graph is substantially denser and street based',()=>{assert.ok(BERLIN.visualEdges.length>240,`visual edges ${BERLIN.visualEdges.length}`);assert.ok(BERLIN.nodes.length>600,`nodes ${BERLIN.nodes.length}`);assert.ok(BERLIN.streetCatalog.length>45,`streets ${BERLIN.streetCatalog.length}`);for(const name of['Kurfürstendamm','Friedrichstraße','Karl-Marx-Allee','Skalitzer Straße','Sonnenallee','Hermannstraße','Schönhauser Allee'])assert.ok(BERLIN.streetCatalog.includes(name),name);});

test('address pool contains street number postcode and unique labels',()=>{assert.ok(BERLIN.addressNodes.length>450,`addresses ${BERLIN.addressNodes.length}`);const labels=new Set();for(const n of BERLIN.addressNodes){assert.equal(n.kind,'address');assert.ok(n.streetName);assert.ok(Number.isInteger(n.houseNumber)&&n.houseNumber>0);assert.match(n.postcode,/^\d{5}$/);assert.match(n.addressLabel,/\s\d+$/);labels.add(n.addressLabel);}assert.equal(labels.size,BERLIN.addressNodes.length);});

test('dense graph is connected from the dispatch base',()=>{const index=createGraphIndex(BERLIN.nodes,BERLIN.edges),start=BERLIN.landmarks.find((l)=>l.id==='checkpoint').addressNodeId;for(const n of BERLIN.addressNodes.filter((_,i)=>i%7===0)){const path=shortestPathIndexed(index,start,n.id);assert.ok(path.length>0,`unreachable ${n.addressLabel}`);}});

test('same seed reproduces city conditions roster goals and initial contracts',()=>{const a=new Game({seed:'REPRO-1'}),b=new Game({seed:'REPRO-1'});assert.equal(a.runTrait.id,b.runTrait.id);assert.equal(a.runContract.id,b.runContract.id);assert.deepEqual(a.couriers.map((c)=>[c.name,c.personality.id,c.experience.level]),b.couriers.map((c)=>[c.name,c.personality.id,c.experience.level]));assert.deepEqual(a.goals,b.goals);assert.deepEqual(a.deliveries.map((d)=>[d.type,d.pickupAddress,d.dropoffAddress,d.reward]),b.deliveries.map((d)=>[d.type,d.pickupAddress,d.dropoffAddress,d.reward]));});
...
test('every generated delivery is an address-to-address contract',()=>{const g=new Game({seed:'ADDRESSES'});for(let i=0;i<20;i++)g.spawnDelivery();for(const d of g.deliveries){assert.match(d.pickupAddress,/\s\d+$/);assert.match(d.dropoffAddress,/\s\d+$/);assert.notEqual(d.pickupAddress,d.dropoffAddress);assert.ok(d.plannedDistance>0);assert.ok(d.plannedStreets.length>0);assert.ok(Object.hasOwn(DELIVERY_TYPES,d.type));}});

test('delivery cargo remains independent from district identity',()=>{const g=new Game({seed:'CARGO'}),seen=new Map();for(let i=0;i<180;i++){g.deliveries=[];g.spawnDelivery();const d=g.deliveries[0];if(!d)continue;const set=seen.get(d.pickupDistrict)??new Set();set.add(d.type);seen.set(d.pickupDistrict,set);}assert.ok(seen.size>=7);for(const set of seen.values())assert.ok(set.size>=2);});
...
test('street goal advances from matching real street endpoint',()=>{const g=new Game({seed:'STREET-GOAL'}),goal=g.goals.find((x)=>x.type==='street'),address=g.addressNodes.find((n)=>n.streetName===goal.targetId),other=g.addressNodes.find((n)=>n.streetName!==goal.targetId),before=goal.progress;g.updateGoals({pickupId:address.id,dropoffId:other.id,edgesTraversed:[]});assert.equal(goal.progress,before+1);});

test('bridge goal only advances for a traversed bridge edge',()=>{const g=new Game({seed:'BRIDGE-GOAL'}),goal=g.goals.find((x)=>x.type==='bridge'),edge=g.edges.find((e)=>e.bridgeId===goal.targetId);assert.ok(edge);const p=g.addressNodes[0],q=g.addressNodes[1],before=goal.progress;g.updateGoals({pickupId:p.id,dropoffId:q.id,edgesTraversed:[edge.id]});assert.equal(goal.progress,before+1);});
