...
test('feasibility UI loads through the read-only outlook layer and exposes four compact states',()=>{assert.match(outlook,/import '\.\/ui-feasibility\.js'/);assert.match(source,/deliveryFeasibility/);for(const label of['AT RISK','delivery slack','No rider projected'])assert.match(source,new RegExp(label));for(const state of['future','tight','risk'])assert.match(css,new RegExp(`data-feasibility=${state}`));});
